...
export const DEPENDENCIES = declareDependencies<PyAgentMetadata>()({
  ...
  py: [
    ...
    // Provides DynamoDBSaver (with S3 offloading for large checkpoints).
    {
      ...
      when: (m) => m.framework === 'langchain' && m.session === 'dynamodb',
    },
    {
      name: 'ag-ui-protocol',
...
export const pyAgentGenerator = async (
  ...
  // while LangChain's LangGraph checkpointer only has a DynamoDB-backed one
  // (via langgraph_checkpoint_aws.DynamoDBSaver, with S3 offloading for large
  // checkpoints). 'in-memory' is valid for both.
  const SESSIONS_BY_FRAMEWORK: Record<
    PyAgentFramework,
    readonly PyAgentSession[]
  > = {
    strands: ['s3', 'in-memory'],
    langchain: ['dynamodb', 'in-memory'],
  };
  if (
    options.session &&
  ...
  const session =
    options.session ?? (framework === 'langchain' ? 'in-memory' : 's3');
