...
`,
  ...
  it('should throw when session s3 is requested for the langchain framework', async () => {
    await expect(
      pyAgentGenerator(tree, {
        project: 'test-project',
      ...
      "Unsupported combination: session 's3' is not implemented for the langchain framework (supported: dynamodb, in-memory).",
    );
  });

  it('should throw when session dynamodb is requested for the strands framework', async () => {
    await expect(
      pyAgentGenerator(tree, {
        project: 'test-project',
      ...
      "Unsupported combination: session 'dynamodb' is not implemented for the strands framework (supported: s3, in-memory).",
    );
  });
  ...
  it('should not throw when session is explicitly dynamodb for the langchain framework', async () => {
    await pyAgentGenerator(tree, {
      project: 'test-project',
  ...
  it('should not throw for the langchain framework when session is left at its default', async () => {
    await pyAgentGenerator(tree, {
      project: 'test-project',
  ...
  it('should not generate session.py for the langchain framework', async () => {
    await pyAgentGenerator(tree, {
      project: 'test-project',
      framework: 'langchain',
      infra: 'none',
  ...
  it('should generate checkpointer.py returning InMemorySaver for the langchain framework', async () => {
    await pyAgentGenerator(tree, {
      project: 'test-project',
      framework: 'langchain',
      infra: 'none',
    ...
    const checkpointerContent = tree.read(
      'apps/test-project/proj_test_project/agent/checkpointer.py',
      'utf-8',
    );
    expect(checkpointerContent).toMatchSnapshot('checkpointer.py (in-memory)');
  });

  it('should generate checkpointer.py using DynamoDBSaver when session is dynamodb', async () => {
    await pyAgentGenerator(tree, {
      project: 'test-project',
      framework: 'langchain',
      session: 'dynamodb',
    ...
    expect(checkpointerContent).toMatchSnapshot('checkpointer.py (dynamodb)');
  });

  it('should not generate checkpointer.py for the strands framework', async () => {
    await pyAgentGenerator(tree, {
      project: 'test-project',
      infra: 'none',
      iac: 'cdk',
    ...
    expect(
      tree.exists('apps/test-project/proj_test_project/agent/checkpointer.py'),
    ).toBeFalsy();
  });
});
